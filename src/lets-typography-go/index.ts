  ...
  blockElements.forEach((item: HTMLElement) => {
    ...
    editElements.forEach((editElement: HTMLElement) => {
      ...
      if (hasElementChildren) {
        ...
        let formatContent = formatUtil.formatContent(editElement.textContent);
        //若内容未变更，不更新
      ...
      imageCenter(editElement, doOperations);
    });
    doOperations.push({
...
export async function imageCenter(editElement, doOperations) {
  ...
  images.forEach(async (item) => {
    //居中
    item.setAttribute("style", `width: min-width: calc(100% - 0.1em);;`);
...
export default class TypographyGo extends SubPluginBase {
  id = "hqweay-format-note";
  get label() {
    return this.t("lets-typography.label");
  }
  ...
  override onload(): void {
    if (settings.getBySpace("typography", "slashFormat")) {
      addProtyleSlash({
        filter: ["fcb"],
        html: this.t("lets-typography.menuItem"),
  ...
  public blockIconEvent({ detail }: any) {
    detail.menu.addItem({
      iconHTML: "",
      label: this.t("lets-typography.label"),
      click: () => {
        formateElement(detail.blockElements, detail.protyle);
      },
    });
  }
  ...
  async exec() {
    ...
    const closeTip = settings.getBySpace("typography", "closeTip");
    if (closeTip) {
      await this.formatTitle();
      await this.formatDoc(parentId);
    } else {
      confirm(
        this.t("lets-typography.warningMsg"),
        this.t("lets-typography.confirmMsg"),
        async () => {
          await this.formatTitle();
          await this.formatDoc(parentId);
        },
  ...
  async formatTitle() {
    const originTitle =
      this.customEvent.detail.protyle.title.editElement.textContent.trim();
    ...
    await fetchSyncPost("/api/filetree/renameDoc", {
      notebook: this.customEvent.detail.protyle.notebookId,
      path: this.customEvent.detail.protyle.path,
      title: title,
    });
  }
  ...
  async formatDoc(parentId) {
    ...
    for (let i = 0; i < childrenBlocks.length; i++) {
      if (i === 0 || i % formatCount === 0) {
        showMessage(
          this.t("lets-typography.progressMsg")
            .replace("{start}", (i + 1).toString())
            .replace("{end}", Math.min(i + formatCount, childrenBlocks.length).toString())
        );
      }
    ...
    const autoSpace = settings.getBySpace("typography", "autoSpace");
    ...
    showMessage(this.t("lets-typography.doneMsg"));
  }
}

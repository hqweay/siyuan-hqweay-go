<script lang="ts">
  ...
  function clickExpandAllListItemNode(event: MouseEvent) {
    ...
    expandAllListItemNode(parentLiElement.nextElementSibling as HTMLElement);
  }
  ...
  async function initBaseData() {
    ...
    hideBacklinkProtyleBreadcrumb = settingConfig.hideBacklinkProtyleBreadcrumb;
    ...
    if (backlinkFilterPanelBaseData && backlinkFilterPanelBaseData.userCache) {
    ...
    let defaultPanelCriteria =
      await BacklinkFilterPanelAttributeService.ins.getPanelCriteria(rootId);

    queryParams = defaultPanelCriteria.queryParams;
    panelFilterViewExpand = defaultPanelCriteria.backlinkPanelFilterViewExpand;
    // panelBacklinkViewExpand =
  ...
  async function pageTurning(pageNumParam: number) {
    ...
    let pageBacklinkPanelRenderData = await getBacklinkPanelRenderData(
      backlinkFilterPanelBaseData,
      queryParams
    );
    ...
    backlinkFilterPanelRenderData.pageNum = pageBacklinkPanelRenderData.pageNum;
  ...
  async function refreshFilterDisplayData() {
    let curDocDefBlockArray = backlinkFilterPanelRenderData.curDocDefBlockArray;
  ...
  function hideOtherListItemElement(
    ...
    let includeChildListItemIdArray = backlinkData.includeChildListItemIdArray;
  ...
  function createdDocumentLiElement(
    ...
    documentLiElement.setAttribute("data-backlink-block-id", backlinkBlockId);
    if (docAriaText) {
...
`;
    ...
    documentLiElement.addEventListener("contextmenu", (event: MouseEvent) => {
...
<div class="backlink-panel__area">
  ...
  <div class="backlink-panel__header">
    ...
    >
      ...
      <span
        ...
        on:keydown={handleKeyDownDefault}
        ><svg class=""><use xlink:href="#iconResetInitialization"></use></svg
        ></span
      >
      <span class="fn__space"></span>
      <span class="fn__space"></span>
      ...
      <div class="fn__flex" style="padding: 5px 15px; maragin:0px;">
        ...
        >
          {#each RELATED_DEF_BLOCK_TYPE_ELEMENT() as element}
            <option
              value={element.value}
              selected={element.value == queryParams.backlinkCurDocDefBlockType}
            >
              {element.name}
            </option>
          {/each}
    ...
    {#if panelBacklinkViewExpand && backlinkFilterPanelRenderData && isArrayNotEmpty(backlinkFilterPanelRenderData.backlinkDataArray)}
      ...
      >
        ...
        {#if SettingService.ins.SettingConfig.scrollLoading}
          <!-- 滚动加载模式 -->
          {#if isLoadingMore}
            <span class="fn__flex-shrink ft__selectnone"> 加载中... </span>
          {:else if !hasMoreData}
            <span class="fn__flex-shrink ft__selectnone"> 已加载全部 </span>
          {:else}
            <span class="fn__flex-shrink ft__selectnone"> 滚动加载更多 </span>
          {/if}
        {:else}
          <!-- 翻页加载模式 -->
          <span
  ...
  <div
    class="backlinkList fn__flex-1 {panelBacklinkViewExpand ? '' : 'fn__none'}"
  >

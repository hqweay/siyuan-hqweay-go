...
export default class DiaryTools extends SubPluginBase {
  private id = "hqweay-diary-tools";
  get label() { return this.t("lets-how-to-write-diary.getWeatherMenu"); }
  ...
  override onload(): void {
    ...
    if (settings.getBySpace("diaryTools", "slashDiaryNote")) {
      addProtyleSlash({
        filter: ["cdn"],
        html: this.t("lets-how-to-write-diary.slashCreateRef"),
        id: "create-daily-note-ref",
        callback: async (event, node) => {
          datePickerDialog({
            title: this.t("lets-how-to-write-diary.datePickerTitle"),
    ...
    if (settings.getBySpace("diaryTools", "topBar")) {
      ...
      menu.addItem({
        ...
        click: async () => {
          ...
          if (!cityCode) {
            showMessage(this.t("lets-how-to-write-diary.noCityCode"), 2000);
            return;
          }

          const attrs = await getBlockAttrs(docID);
          if (attrs["custom-diary-weather-type"]) {
            showMessage(this.t("lets-how-to-write-diary.hasWeatherAttr"), 2000);
            return;
          }
          ...
          showMessage(this.t("lets-how-to-write-diary.weatherInserted"), 2000);
        },
      });
    }
  }

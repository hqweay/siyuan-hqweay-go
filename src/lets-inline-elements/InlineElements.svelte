<script lang="ts">
  ...
  import { openBlockByID } from "@/myscripts/syUtils";
    ...
    export let useProtyle: boolean = false;
    ...
    // 过滤显示的元素
    $: filteredElements = inlineElements.filter(element => 
        !searchText || 
    ...
    <div class="panel-header">
        <div class="panel-title">
            <svg class="panel-icon" width="16" height="16" viewBox="0 0 24 24">
                <path fill="currentColor" d="M17.63 5.84C17.27 5.33 16.67 5 16 5L5 5.01C3.9 5.01 3 5.9 3 7v10c0 1.1.9 2 2 2h11c.67 0 1.27-.33 1.63-.84L22 12L17.63 5.84z"/>
            </svg>
            <span>文档标注</span>
            {#if useProtyle}
                <span class="mode-badge protyle">Protyle</span>
            {:else}
                <span class="mode-badge simple">简单</span>
            {/if}
        </div>
        <div class="element-count">{paginatedElements.length}{useProtyle ? `/ ${filteredElements.length}` : ''}</div>
    </div>
                        ...
                        <div class="protyle-element-item" data-element-id={element.id}>
                            <div class="element-header">
                                ...
                                <div class="element-info">
                                    <div class="element-id" on:click={() => copyToClipboard(element.id)}>
                                        {element.id}
                                    </div>

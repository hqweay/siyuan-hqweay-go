...
} from "@/lets-syplugin-backlink-panel/service/setting/BacklinkPanelFilterCriteriaService";
...
async function getDocumentBottomBacklinkPanelDisplay(
  ...
  if (isCardBlock) {
    let flashCardBottomDisplay =
      SettingService.ins.SettingConfig.flashCardBottomDisplay;
...
async function addBacklinkPanelToBottom(
  ...
  let docBottomBacklinkPanelViewExpand =
  ...
  backlinkPanelBottomElement.addEventListener(
    "mouseover",
    (event: MouseEvent) => {
